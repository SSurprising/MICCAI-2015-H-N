...
def main():
    ...
    if args.test:
        ...
        train_ds = Dataset(cts=train_ct_list, gts=train_gt_list, crop_size=args.crop_size, random_crop=True)
...
def train(net, train_dl, loss_func, opt):
    ...
    for step, (ct, seg) in enumerate(train_dl):
        loss = 0
        if args.model_name == 'SC_UNet_all':
            outputs_1, outputs_2 = net(ct.cuda())
            loss += args.lw * loss_func(outputs_1, seg.cuda()) + (1 - args.lw) * loss_func(outputs_2, seg.cuda())

        elif args.model_name == 'SC_UNet' or 'Cascaded_UNet':
            outputs_1, outputs_2 = net(ct.cuda())

            if args.auxiliary_loss:
                loss_1 = auxiliary_loss(loss_func, outputs_1, seg.cuda())
                loss_2 = auxiliary_loss(loss_func, outputs_2, seg.cuda())
            else:
                loss_1 = loss_func(outputs_1[-1], seg.cuda())
                loss_2 = loss_func(outputs_2[-1], seg.cuda())
            loss += args.lw * loss_1 + (1 - args.lw) * loss_2
        else:
            outputs = net(ct.cuda())
...
def val(net, save_path=None, size=128):
    net.eval()

    dice_list = []
    validate_path = '/home/zjm/Data/HaN_OAR_raw/four_organ_fast_validate/'
...
def test(net, save_path=None, size=128):
    ...
    for root, dirs, files in os.walk(args.data_path):
        if len(files) and args.val_subset in root:
        # if len(files) and 'subset1' in root:
            print(root)
            seg_list = []
            ...
            # This only for evaluating my modification
            if False:
                idx = np.where(gt_arr != 0)
                img_arr = img_arr[min(idx[0])-4:max(idx[0])+1+4, ::]
                gt_arr = gt_arr[min(idx[0])-4:max(idx[0])+1+4, ::]
            ...
            dice_score_patient_pp = dice_score(pred_seg_full_pp, gt_arr,
